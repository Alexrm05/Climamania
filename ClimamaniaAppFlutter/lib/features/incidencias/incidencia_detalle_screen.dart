...
class _View extends StatelessWidget {
          ...
          if (c.detalle == null) {
            return Center(
              child: Padding(
                ...
                child: Text(c.errorMsg ?? 'No se pudo cargar la incidencia',
  ...
  Widget _datosCard(
      BuildContext context, IncidenciaDetalleController c, GestionItem v) {
    final prio = Prioridad.parse(v.prioridad);
    final dir = [
      if (v.direccion.isNotEmpty) v.direccion,
      if (v.poblacion.isNotEmpty) v.poblacion,
    ].join(', ');
    final titulo = v.referencia.isNotEmpty
        ? v.referencia
        : (v.cliente.isNotEmpty ? v.cliente : 'Incidencia #${v.id}');
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          if (v.cliente.isNotEmpty && v.referencia.isNotEmpty)
            _line('Cliente', v.cliente),
          ...
          SizedBox(
            ...
            child: ElevatedButton.icon(
              ...
              label: const Text('Gestionar incidencia'),
              onPressed: () async {
    ...
    return Padding(
      ...
      child: Container(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Text('Historial de la incidencia',
                style: TextStyle(
                    fontSize: 15,
                    fontWeight: FontWeight.bold,
                    color: AppColors.primaryDark)),
