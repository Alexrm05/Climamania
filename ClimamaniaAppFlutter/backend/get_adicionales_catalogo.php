...
function fetchProductsAndPricing(PDO $pdo, string $prefix, int $shopId, int $langId, array $ids, array $categoryIds): array
{
    ...
    $sql = "SELECT
                ...
                (
                    CASE
                        WHEN sp1.price IS NOT NULL AND sp1.price <> -1
                            THEN sp1.price
                        ELSE ps.price
                    END
                    + IFNULL(comb.impacto, 0)
                ) AS precio_sin_iva,
            ...
            LEFT JOIN " . ident($prefix . "specific_price") . " sp1
                ON p.id_product = sp1.id_product
               AND sp1.id_group = 0
               AND sp1.id_customer = 0
               AND sp1.from_quantity = 1
            LEFT JOIN (
            ...
            SELECT
                ...
                (
                    CASE
                        WHEN sp.price IS NOT NULL AND sp.price <> -1
                            THEN sp.price
                        ELSE ps.price
                    END
                    + IFNULL(comb.impacto, 0)
    ...
    foreach ($rows as $row) {
        ...
        $linePrice = formatDecimal(normalizeDecimal($row["precio_sin_iva"] ?? ($row["precio_sin_Iva"] ?? "0")), 6);
