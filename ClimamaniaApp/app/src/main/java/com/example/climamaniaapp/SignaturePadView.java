...
public class SignaturePadView extends View {
    ...
    public interface OnSignatureStateChangedListener {
        void onSignatureStateChanged(boolean hasSignature);
    }
    ...
    @Nullable
    private OnSignatureStateChangedListener signatureStateChangedListener;
    ...
    private void init() {
        ...
        strokePaint.setColor(Color.parseColor("#1565C0"));
    ...
    public boolean onTouchEvent(@NonNull MotionEvent event) {
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                lastX = x;
                lastY = y;
                boolean wasSigned = hasSignature;
                hasSignature = true;
                if (!wasSigned) {
    ...
    public void setOnSignatureStateChangedListener(@Nullable OnSignatureStateChangedListener listener) {
        this.signatureStateChangedListener = listener;
    }
    ...
    private void notifySignatureStateChanged() {
        if (signatureStateChangedListener != null) {
            signatureStateChangedListener.onSignatureStateChanged(hasSignature);
        }
    }
}
