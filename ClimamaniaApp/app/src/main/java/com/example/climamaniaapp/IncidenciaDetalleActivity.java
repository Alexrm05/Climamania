...
public class IncidenciaDetalleActivity extends BaseActivity {

    private static final String INCIDENCIA_DETALLE_URL = "https://app.clminstal.es/api/get_incidencia_detalle.php";
    ...
    private JSONObject incidenciaActual;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        TextView txtTitulo = content.findViewById(R.id.txtDetalleVisitaTitulo);
        TextView txtMuroTitulo = content.findViewById(R.id.txtDetalleVisitaMuroTitulo);
        TextView txtFicherosTitulo = content.findViewById(R.id.txtDetalleVisitaFicherosTitulo);
        ...
        if (btnEnviarComercial != null) {
            ...
            btnEnviarComercial.setText("Gestionar incidencia");
        }
        if (txtTitulo != null) {
            txtTitulo.setText("Detalle incidencia");
        }
        if (txtMuroTitulo != null) {
            txtMuroTitulo.setText("Historial de la incidencia");
        }
        if (txtFicherosTitulo != null) {
            txtFicherosTitulo.setText("Ficheros y fotos");
        }
        ...
        idIncidencia = UiText.sanitizeDbValue(getIntent().getStringExtra("id_incidencia"));
        if (idIncidencia.isEmpty()) {
            idIncidencia = UiText.sanitizeDbValue(getIntent().getStringExtra("id_visita"));
        }
        referenciaInstalacion = UiText.sanitizeDbValue(getIntent().getStringExtra("referencia"));
        if (idIncidencia.isEmpty()) {
        ...
        if (txtRef != null) {
            txtRef.setText("Incidencia #" + idIncidencia);
        }
    ...
    protected void onResume() {
        super.onResume();
        if (reloadAfterActions && idIncidencia != null && !idIncidencia.isEmpty()) {
    ...
    protected void onDestroy() {
        ...
        if (requestQueue != null) {
            requestQueue.cancelAll("incidencia_detalle");
            requestQueue.cancelAll("incidencia_detalle_img");
        }
    }
    ...
    private void cargarDetalleIncidencia() {
        if (txtLoading != null) {
            ...
            txtLoading.setText("Cargando detalle de la incidencia...");
        }
        ...
        try {
            String url = INCIDENCIA_DETALLE_URL
                    + "?api_key=" + URLEncoder.encode(API_KEY, "UTF-8")
                    + "&id_incidencia=" + URLEncoder.encode(idIncidencia, "UTF-8")
                    + "&rol=" + URLEncoder.encode(rol, "UTF-8")
                    + "&equipo=" + URLEncoder.encode(equipo, "UTF-8")
                    + "&usuario=" + URLEncoder.encode(usuario, "UTF-8")
                    + "&_ts=" + System.currentTimeMillis();
            if (!referenciaInstalacion.isEmpty()) {
                url += "&referencia=" + URLEncoder.encode(referenciaInstalacion, "UTF-8");
            }
            ...
            StringRequest request = new StringRequest(
                    ...
                    error -> showError("No se pudo cargar el detalle de la incidencia"));
            request.setShouldCache(false);
            request.setTag("incidencia_detalle");
            requestQueue.add(request);
        } catch (Exception e) {
            showError("No se pudo cargar el detalle de la incidencia");
        }
    }
    ...
    private void handleDetalleResponse(String response) {
        try {
            ...
            if (!root.optBoolean("success", false)) {
                ...
                showError(message.isEmpty() ? "No se pudo cargar el detalle de la incidencia" : message);
                return;
            }

            JSONObject incidencia = root.optJSONObject("incidencia");
            if (incidencia == null) {
                incidencia = root.optJSONObject("visita");
            }
            if (incidencia == null) {
                showError("Detalle de incidencia no disponible");
                return;
            }

            String refServer = UiText.sanitizeDbValue(incidencia.optString("referencia", ""));
            if (!refServer.isEmpty()) {
                referenciaInstalacion = refServer;
            }

            incidenciaActual = incidencia;
            bindIncidencia(incidencia);
            ...
            showError("Respuesta invalida al cargar detalle de incidencia");
        }
    }

    private void bindIncidencia(JSONObject incidencia) {
    ...
    private void loadImagePreview(List<String> urls, int index, ImageView imageView, String[] resolvedUrl) {
        ...
        imageRequest.setTag("incidencia_detalle_img");
    ...
    private boolean isImageFile(String path, String tipoFichero) {
        ...
        if (value.endsWith(".jpg") || value.endsWith(".jpeg") || value.endsWith(".png")
                ...
                || value.endsWith(".heic") || value.endsWith(".heif")) {
            return true;
        }
        ...
        return tipo.contains("image") || tipo.contains("foto") || tipo.contains("jpg")
                ...
                || tipo.contains("heic") || tipo.contains("heif");
    }
    ...
    private void abrirPantallaEnviar() {
        Intent intent = new Intent(this, IncidenciaEnviarActivity.class);
        intent.putExtra("id_incidencia", idIncidencia);

        if (incidenciaActual != null) {
            intent.putExtra("cliente", UiText.sanitizeDbValue(incidenciaActual.optString("cliente", "")));
            intent.putExtra("direccion", UiText.sanitizeDbValue(incidenciaActual.optString("direccion", "")));
            intent.putExtra("poblacion", UiText.sanitizeDbValue(incidenciaActual.optString("poblacion", "")));
            intent.putExtra("referencia", UiText.sanitizeDbValue(incidenciaActual.optString("referencia", "")));
        }
    ...
    private void showError(String message) {
        ...
        if (msg.isEmpty()) {
            msg = "No se pudo cargar el detalle de la incidencia";
        }
        if (txtLoading != null) {
