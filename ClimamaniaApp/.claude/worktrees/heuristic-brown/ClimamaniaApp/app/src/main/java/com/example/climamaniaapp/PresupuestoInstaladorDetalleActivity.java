...
public class PresupuestoInstaladorDetalleActivity extends BaseActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (btnPdf != null) {
            btnPdf.setOnClickListener(v -> openUrl(pdfUrl));
        }
        if (btnFirma != null) {
    ...
    private String parseDetalleResponse(String response, String sourceLabel) {
        try {
            ...
            bindLineas(lineas);
            canEdit = root.optBoolean("can_edit", false);
    ...
    private void recalculateLine(EditableLine line) {
        ...
        line.totalConIva = line.totalSinIva.multiply(ivaMultiplier).setScale(6, RoundingMode.HALF_UP);
    ...
    private void refreshLineAmounts(TextView txtUnit, TextView txtTotal, TextView txtIvaInfo, EditableLine line) {
        ...
        String ivaLabel = "IVA " + formatPercent(line.ivaPct) + " · Total c/IVA " + formatCurrency(line.totalConIva);
        if (line.ivaFallback) {
            ivaLabel += " (estimado)";
        }
