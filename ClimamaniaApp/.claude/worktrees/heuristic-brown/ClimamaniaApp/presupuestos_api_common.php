...
{
    $clean = presup_normalize_media_path($ruta);
    if ($clean === "") {
        return "";
    }
    if (stripos($clean, "http://") === 0 || stripos($clean, "https://") === 0) {
...
{
    $clean = presup_normalize_media_path($storedPath);
    if ($clean === "") {
        return "";
    }
...
function presup_build_nota_cargo_pdf(
    ...
    $rows = [];
    foreach ($lineas as $linea) {
        $desc = presup_pdf_clean_for_document((string)($linea["descripcion"] ?? ""));
        if ($desc === "") {
            $desc = "Sin descripcion";
        }
        $descLines = presup_pdf_wrap_for_width($desc, $tableWidths[2] - 8.0, 9.0);
        if (empty($descLines)) {
            $descLines = [$desc];
        }

        $rowHeight = max($minRowHeight, (count($descLines) * $rowLineHeight) + 8.0);
        $rows[] = [
            "cantidad" => (float)($linea["cantidad"] ?? 0),
            "articulo" => presup_pdf_clean_for_document((string)($linea["articulo"] ?? "")),
            "descripcion_lineas" => $descLines,
            "precio_unitario_sin_iva" => (float)($linea["precio_unitario_sin_iva"] ?? 0),
            "precio_total_linea" => (float)($linea["precio_total_linea"] ?? 0),
    ...
    if (empty($rows)) {
        $rows[] = [
            ...
            "precio_unitario_sin_iva" => 0.0,
            "precio_total_linea" => 0.0,
...
function presup_pdf_draw_nota_table_header(
    ...
    $content .= presup_pdf_text_cmd($tableX + $widths[0] + $widths[1] + $widths[2] + 4.0, $y - 15.0, "Neto ud", 9.0, true, "C", $widths[3] - 8.0);
...
function presup_pdf_draw_nota_row(
    ...
    $middleY = $y - ($rowHeight / 2.0) - 2.0;
    $content .= presup_pdf_text_cmd(
        $tableX + 4.0,
        $middleY,
        presup_format_qty_pdf((float)($row["cantidad"] ?? 0)),
        9.0,
        false,
        "C",
        $widths[0] - 8.0
    ...
    $descLines = $row["descripcion_lineas"] ?? ["-"];
    $descY = $y - 13.0;
    foreach ($descLines as $descLine) {
        if ($descY < ($y - $rowHeight + 7.0)) {
            break;
        }
        $content .= presup_pdf_text_cmd(
    ...
    $content .= presup_pdf_text_cmd(
        ...
        $middleY,
        presup_format_eur_pdf((float)($row["precio_unitario_sin_iva"] ?? 0)),
        9.0,
        false,
        "R",
        $widths[3] - 8.0
    ...
    $content .= presup_pdf_text_cmd(
        ...
        $middleY,
        presup_format_eur_pdf((float)($row["precio_total_linea"] ?? 0)),
        9.0,
        false,
        "R",
        $widths[4] - 8.0
...
function presup_build_email_body(
    ...
    return "<!doctype html><html><body style=\"font-family:Arial,Helvetica,sans-serif;color:#111;\">"
        ...
        . "<th style=\"border:1px solid #444;padding:6px;background:#efefef;\">P.unit</th>"
